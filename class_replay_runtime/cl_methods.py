...
@contextmanager
def freeze_backbone_bn(backbone):
    layers = [m for m in backbone.modules() if isinstance(m, torch.nn.modules.batchnorm._BatchNorm)]
    modes = [m.training for m in layers]
    try:
        for m in layers:
            m.eval()
        yield
    finally:
        for m, mode in zip(layers, modes):
            m.training = mode


def stable_features(model, images):
...
class DarkExperienceReplayPlus:
    ...
    def feature_penalty(self, model, device: torch.device) -> tuple[torch.Tensor, tuple[torch.Tensor, ...] | None]:
        ...
        if self.alpha == 0:
            return next(model.parameters()).new_zeros(()), replay
        examples, targets = replay[:2]
        features = (stable_features(model, examples) if getattr(self, "freeze_replay_bn", False)
                    else model.backbone(examples))
        if features.shape != targets.shape:
            raise ValueError("DER++ replay feature shape mismatch")
