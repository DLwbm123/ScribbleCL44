...
if "--setting-run" in sys.argv:
    sys.argv.remove("--setting-run")
    from runner import main as setting_main
    setting_main()
    raise SystemExit(0)
...
def main(args):
    ...
    args.mean = torch.tensor([0.5], dtype=torch.float32).reshape(1,1,1,1).to(args.device)
    args.std = torch.tensor([0.5], dtype=torch.float32).reshape(1,1,1,1).to(args.device)
    print(args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        optimizer.param_groups[0]['lr'] = 1e-4#clr.cyclic_learning_rate(epoch, mode='exp_range', gamma=1)
        # if args.distributed:
        #     sampler_train.set_epoch(epoch)
        estimate_alpha = False
        if epoch > 60:
            estimate_alpha = True
        train_stats = train_one_epoch(model, criterion, dataloader_train_dict, optimizer, device, epoch, args,writer, estimate_alpha)
        ## lr_scheduler
        # lr_scheduler.step()
