...
def organ_training_policy(args, task_code):
    ...
    if args.organ_tuning and task_code != 'T1':
        ...
        policy.update(backbone_lr_scale=args.organ_backbone_lr_scales[task_number-2],
                      ...
                      grad_clip_norm=5.)
    if getattr(args, "method", "") in ("zs-er", "zs-der"):
        # These controls retain their declared loss across all task transitions.
        policy.update(der_alpha=args.der_alpha, der_beta=args.der_beta)
    return policy
...
def derpp_replay_losses(
    ...
    with freeze_batchnorm_stats(model.backbone):
        for task_value in torch.unique(task_ids, sorted=True).tolist():
            ...
            if args.zs_global_weight and getattr(args, "method", "") != "zs-er":
    ...
    try:
        ...
        return {
            **diagnostics,
            ...
            "background_inclusive_dice": float(inclusive_values.mean()),
...
def main(project_scenario: str) -> None:
    ...
    # Reuse paired replay storage for ER and feature-DER in the new controls.
    replay_control = args.method in ("zs-er", "zs-der") and project_scenario in ("organ", "domain")
    use_der = args.method.endswith("-der") and not replay_control
    use_derpp = args.method.endswith("-derpp") or replay_control
    ...
    if args.organ_tuning and (project_scenario != 'organ'
            or args.method not in ('zs-sequential','zs-ewc','zs-gpm','zs-er','zs-der')
    ...
    manifest = {
        ...
        "derpp_target": ("sparse_pce_only" if args.method == "zs-er" else
                         "backbone_features_only" if replay_control else
                         "backbone_features_plus_sparse_pce_global") if use_derpp else None,
        "replay_storage": "paired reservoir; unaugmented images; feature targets with BN frozen" if use_derpp else None,
    ...
    if project_scenario == "domain" and args.test_evaluation:
        ...
        random_scores = [score["benchmark_mean"] for score in random_evaluations]
        random_inclusive_scores = [score["background_inclusive_dice"] for score in random_evaluations]
        (args.output / "random_scores.json").write_text(json.dumps({
            "seed": args.seed,
            "scores": random_scores,
            "background_inclusive_scores": random_inclusive_scores,
            ...
            for epoch in range(executed_epochs):
                ...
                for batch_index, batch in enumerate(train_loader):
                    ...
                    if use_derpp:
                        ...
                        if replay is None or replay_beta == 0:
                    ...
                    if use_derpp:
                        with torch.no_grad():
                            ...
                            feature_targets = (replay_image.new_empty((len(replay_image), 0))
                                if args.method == "zs-er" else
    ...
    inclusive_matrix = [
        [row["evaluated"].get(task.code, {}).get("background_inclusive_dice") for task in tasks]
        for row in stage_rows
    ]
    summary = {
        "background_inclusive_matrix": inclusive_matrix,
        "final_background_inclusive_mean": (float(np.mean(inclusive_matrix[-1][:last_stage+1]))
                                            if args.test_evaluation else None),
        "random_background_inclusive_scores": random_inclusive_scores,
        "method": args.method,
