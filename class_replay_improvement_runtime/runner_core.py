...
class H5Slices(Dataset):
    ...
    def __init__(
        ...
        if split == "train" and not dense_supervision:
            ...
            if sparse_label_shift:
                self.sparse = np.where(self.sparse > 0, self.sparse - sparse_label_shift, self.sparse)
...
class ClassModel(nn.Module):
    ...
    def __init__(self, block_sizes=None) -> None:
        super().__init__()
    ...
    def activate_stage(self, stage: int) -> None:
        ...
        if getattr(self, "improved_replay", False) and getattr(self, "lock_old_heads", False) and stage > 0:
            for head in [self.background, *self.blocks[:stage]]:
                head.requires_grad_(False)

    def train(self, mode=True):
        super().train(mode)
        if getattr(self, "improved_replay", False) and self.active_stage > 0:
...
def partial_background_pce(probabilities, labels, foreground_classes):
    """Other task classes are unknown within this task's annotated background."""
    known = labels.ne(IGNORE_INDEX)
    allowed = labels.eq(IGNORE_INDEX) | labels.eq(0)
    for value in foreground_classes:
...
@torch.no_grad()
def refresh_feature_targets(model, buffer, device):
    if buffer is None or buffer.alpha == 0 or not len(buffer):
        return
    was_training = model.training
    model.eval()
    for start in range(0, len(buffer), buffer.minibatch_size):
        images = torch.stack(buffer.examples[start:start + buffer.minibatch_size]).to(device)
        features = model.backbone(images).cpu()
        if not torch.isfinite(features).all():
            raise FloatingPointError("non-finite refreshed feature target")
        buffer.feature_targets[start:start + len(images)] = list(features.unbind())
...
def derpp_replay_losses(
    ...
    if getattr(args, "method", "") == "zs-er":
        with freeze_backbone_bn(model.backbone):
            outputs = zs_forward(model, examples, None)
        if getattr(args, "improved_replay", False):
            loss = examples.new_zeros(())
            for task_value in torch.unique(task_ids).tolist():
                mask = task_ids.eq(task_value)
                count = labels[mask].ne(IGNORE_INDEX).sum()
                loss = loss + count * partial_background_pce(
                    outputs["pred_masks"][mask], labels[mask], TASKS["class"][int(task_value)].classes)
            return loss / labels.ne(IGNORE_INDEX).sum().clamp_min(1), examples.new_zeros(())
        target = native_target(labels, model.output_channels(model.active_stage))
        return pce_loss(outputs["pred_masks"], target), examples.new_zeros(())
    pce_terms = []
    global_terms = []
...
METHODS = {
    "class": (
        ...
        "zs-mib", "zs-gpm", "zs-der", "zs-derpp", "zs-derpp-mib", "zs-er",
    ),
    "organ": (
...
def main(project_scenario: str) -> None:
    ...
    parser.add_argument("--improved-replay", action="store_true")
    ...
    if args.improved_replay and not replay_control:
        parser.error("improved replay is limited to Class ER/feature-DER")
    ...
    if not 1 <= args.selection_min_epoch <= args.epochs_per_task:
        parser.error("selection-min-epoch must be within the executed training epochs")
    if args.class_independent_task:
        if project_scenario != "class" or args.method != "zs-sequential" or args.max_task is not None:
            parser.error("class independent requires Class zs-sequential without max-task")
        independent_task = next(t for t in tasks if t.code == args.class_independent_task)
        tasks = (Task(independent_task.code, independent_task.folder, independent_task.filename,
                      tuple(range(1, len(independent_task.classes) + 1)), 0),)
    ...
    if args.independent_reference:
        ...
        return
    model = (ClassModel((len(independent_task.classes),)) if independent_task else _build_model(project_scenario))
    model.to(device)
    model.improved_replay = args.improved_replay
    model.lock_old_heads = args.improved_replay and args.method == "zs-der"
    ...
    if replay_control:
        derpp.freeze_replay_bn = True
    args.output.mkdir(parents=True, exist_ok=False)
    manifest = {
        "scenario": project_scenario,
        "improved_replay": args.improved_replay,
        "selection": "seen-task mean foreground validation" if args.improved_replay else "current-task foreground validation",
        "method": args.method,
        "main_entry": "main.py",
        "backbone": "ZScribbleSeg_UNet",
        "seed": args.seed,
        ...
        "independent_global_classes": None if independent_task is None else independent_task.classes,
        ...
        "derpp_target": ("sparse_pce_current_seen_classes" if args.method == "zs-er" else
    ...
    if args.resume_from:
        ...
        if args.improved_replay:
            refresh_feature_targets(model, derpp, device)
        del state
    ...
    for stage, task in enumerate(tasks[:last_stage + 1]):
            ...
            for epoch in range(epochs_this_task):
                ...
                for batch_index, batch in enumerate(train_loader):
                    ...
                    if use_mib and stage > 0:
                        ...
                        partial_ce = (partial_background_pce(outputs["pred_masks"], label, task.classes)
                                      if args.improved_replay else pce_loss(outputs["pred_masks"], target))
                    ...
                    if use_derpp:
                        ...
                        if replay is None or args.der_beta == 0:
                    ...
                    if use_derpp:
                        with torch.no_grad():
                            ...
                            derpp.add_data(
                                image,
                                (image.new_empty((len(image), 0)) if args.method == "zs-er" else
                                 stable_features(model, image).detach() if replay_control else model.backbone(image)),
                                label,
                                replay_task_ids,
                                classes,
                    ...
                    iteration += 1
                    if replay_control and iteration == 2:
                    ...
                    if iteration % args.validate_every == 0:
                        ...
                        if args.improved_replay:
                            old_scores = [_evaluate_task(model, project_scenario, old_task, old_stage,
                                args.data_root, "val", args.batch_size, device)["benchmark_mean"]
                                for old_stage, old_task in enumerate(tasks[:stage])]
                            validation["selection_score"] = float(np.mean([*old_scores, validation["benchmark_mean"]]))
                            validation["old_task_foreground"] = old_scores
                        stream.write(json.dumps({
                            "stage": stage,
                            "epoch": epoch,
                            "iteration": iteration,
                        ...
                        stream.flush()
                        if epoch + 1 >= args.selection_min_epoch and validation.get("selection_score", validation["benchmark_mean"]) > best.get("selection_score", best["benchmark_mean"]):
                            best = {**validation, "epoch": epoch, "iteration": iteration}
                            torch.save(model.state_dict(), best_path)
                row = {
                    "stage": stage,
        ...
        for evaluated_stage, evaluated_task in enumerate(() if args.validation_only else evaluation_tasks):
        ...
        stage_row = {
            ...
            "seen_validation_mean": float(np.mean(tuple(seen_validation.values()))),
            "evaluated": evaluated,
    ...
    if project_scenario == "class" and not independent_task and not args.validation_only and last_stage == len(tasks) - 1:
