...
trap 'TASK_EXIT=$?; printf "%s\n" "$TASK_EXIT" > "$RUN_ROOT/training60.exitcode"' EXIT
...
' --setting-run --data-root "$RUN_ROOT/subset/data" --sparse-root "$RUN_ROOT/subset/sparse" \
  --output "$RUN_ROOT/run60" --device cuda:0 --seed 42 \
  --epochs-per-task 60 --max-task 3 --batch-size 4 --workers 8 --lr .03 \
