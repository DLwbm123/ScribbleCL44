...
def _match_spatial(skip, reference):
    """Center-crop a skip tensor to the decoder tensor's spatial size."""
...
class Unet(nn.Module):
    ...
    def forward(self, tensor_list):
        ...
        d4 = self.Up4(e5)
        e4_cropped = _match_spatial(e4, d4)
        d4 = torch.cat((d4,e4_cropped), dim=1)
        d4 = self.Up_conv4(d4)

        d3 = self.Up3(d4)
        e3_cropped = _match_spatial(e3, d3)
        d3 = torch.cat((d3,e3_cropped), dim=1)
        d3 = self.Up_conv3(d3)

        d2 = self.Up2(d3)
        e2_cropped = _match_spatial(e2, d2)
        d2 = torch.cat((d2,e2_cropped), dim=1)
        d2 = self.Up_conv2(d2)

        d1 = self.Up1(d2)
        e1_cropped = _match_spatial(e1, d1)
...
def build_UNet(args):
    return Unet(in_ch=1, out_ch=getattr(args, "out_channels", 4))
